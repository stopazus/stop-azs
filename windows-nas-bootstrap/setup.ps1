...
if ($mappedCount -lt $drives.Count) {
    $exitCode = 1
}

# Clear sensitive password from memory after use
if ($null -ne $password) {
    try {
        $password.Dispose()
    } catch {
        # SecureString may not expose Dispose on older PowerShell; fall back to nulling the reference
    }
    $password = $null
}
