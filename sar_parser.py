...
@dataclass
class SARData:
    """High level information extracted from a SAR filing."""
...
def parse_sar(xml_content: str) -> SARData:
    ...
    for tx_el in root.findall("f:Transactions/f:Transaction", _NAMESPACE):
        ...
        if amount_el is not None:
            amount_value = _text_or_none(amount_el.find("f:Value", _NAMESPACE)) or _text_or_none(amount_el)
            amount_currency = amount_el.get("currencyCode") or amount_el.get("currency")

        party_elements: List[ET.Element] = list(tx_el.findall("f:Parties//f:Name", _NAMESPACE))
        if not party_elements:
            party_elements = list(tx_el.findall(".//f:PartyName", _NAMESPACE))
        transactions.append(
            Transaction(
                date=_text_or_none(tx_el.find("f:Date", _NAMESPACE)),
                amount=amount_value,
                currency=amount_currency,
                instrument_type=_text_or_none(tx_el.find("f:InstrumentType", _NAMESPACE)),
                party_names=_collect_text_list(party_elements),
            )
        )

    return SARData(
        filing_id=_text_or_none(root.find("f:FilingIdentification", _NAMESPACE)),
...
def _load_xml(path: Path) -> str:
    """Return the XML content stored at ``path``.

    ``Path.read_text`` is used so ``path`` can point to local or temporary
    files without needing manual file handling.
    """
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    args = list(argv)
    if len(args) != 1:
        sys.stderr.write("usage: sar_parser.py <path-to-sar-xml>\n")
        return 1

    xml_path = Path(args[0])
    if not xml_path.exists():
        sys.stderr.write(f"error: {xml_path} does not exist\n")
        return 2

    xml_content = _load_xml(xml_path)
    sar_data = parse_sar(xml_content)
    json.dump(sar_data.to_dict(), sys.stdout, indent=2, sort_keys=True)
    sys.stdout.write("\n")
    return 0
